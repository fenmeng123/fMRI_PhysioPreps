function [data_table,T_ECG_Channels] = s_tapas_physio_siemens_line2table(lineData, cardiacModality)
...
%% Codes added by Kunru: Part 1

[T_ParsedHeaders,~,~] = s_parse_siemens_ECG_Extract_Headers(lineData);
[FullTraceDat,T_ECG_Channels,T_ECG_SignalIndex] = s_parse_siemens_ECG_Channels(T_ParsedHeaders);
%% Raw codes
if ~exist('FullTraceDat','var')
...
% Check the pairs for cardic pulse on-off
if length(cpulse) ~= length(cpulse_off)
    if cpulse(1) > cpulse_off(1)
        cpulse_off = cpulse_off(2:end);
    end
end
...
iNonEcgSignals = [cpulse; cpulse_off;...
    %========================Deprecated By Kunru==========================%
...
%% Codes added by Kunru:Part 2
% data_stream contains all 4-channels time courses
data_stream = T_ECG_Channels.Variables;
...
switch upper(cardiacModality) % ecg has two channels, resp and puls only one
    case 'ECG'
        ...
        % ----------------------------New Codes Added-----------------------------%
        data_table = zeros(nRows,3);
        data_table(1:nRows,1:2) = data_stream(:,1:2);
        % now fill up 3rd column with trigger data
        % - for each trigger index in data{1}, check where ECG data with closest
        % smaller index ended up in the data_table ... and put trigger code in
        % same row of that table
        ...
        for iTrigger = 1:nTriggers
            % find index before trigger event in data stream and
            % detect it in table
            [iRow,~] = find(T_ECG_SignalIndex.Variables == iNonEcgSignals(iTrigger)-1);
            data_table(iRow,3) = codeNonEcgSignals(iTrigger);
        end
